...
#' \item Quite A number of functions in the \emph{matrixStats} package do not (by default) preserve the attributes of objects passed to them, 
#' resulting in inconsistent behavior of different functions. I have implemented a solution to this in the \code{fastverse}, by writing very efficient wrappers
#' around \emph{matrixStats} functions that do not preserve attributes, and replacing these functions in the \emph{matrixStats} namespace whenever the \code{fastverse}
#' is attached (regardless of whether \emph{matrixStats} was already attached or not). Unfortunately this original release has been rejected by CRAN, because CRAN
#' does not allow packages to modify the namespace of other packages. If you want to use \emph{matrixStats}, and you want consistent attribute handling by default, 
#' I highly recommend installing the development version of the \code{fastverse} from GitHub using \code{remotes::install_github("SebKrantz/fastverse")}. 
#' Once \emph{matrixStats} has evolved so that consistent attribute handling (\code{useNames = TRUE}) becomes the default in all functions, the CRAN and GitHub versions will be
#' the same. 
#' }
#' 
#' 
